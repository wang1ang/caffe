...
void FocalLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
	  ...
	  Dtype prob = prob_data[i * dim + label_value * inner_num_ + j];
      loss -= (1 - prob) * (1 - prob) * log(std::max(prob, Dtype(FLT_MIN))); // gamma = 2
      ++count;
    }
  }
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
		  ...
		  Dtype prob = bottom_diff[i * dim + label_value * inner_num_ + j];
          bottom_diff[i * dim + label_value * inner_num_ + j] = (1-prob) * (1-prob) * (2*prob*log(std::max(prob, Dtype(FLT_MIN)))+prob-1);
          ++count;
        }
      }
    }
