...
void HypercolumnExtractorLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
}

template <typename Dtype>
void HypercolumnExtractorLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  HypercolumnExtractorParameter param = this->layer_param().hypercolumn_extractor_param();
...
template <typename Dtype>
void HypercolumnExtractorLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void HypercolumnExtractorLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    for (int p = 0; p < num_locations_; ++p) {
      ...
      for (int l = 0; l < num_layers_; ++l) {
        ...
        for (int c = 0; c < num_layer_channels; ++c) {
          ...
          Dtype diff = hypercol_diff[offset0 + cc + c];
...
#ifdef CPU_ONLY
STUB_GPU(HypercolumnExtractorPairsLayer);
#endif

INSTANTIATE_CLASS(HypercolumnExtractorLayer);
REGISTER_LAYER_CLASS(HypercolumnExtractor);
